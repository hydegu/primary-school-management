...
**当前实际实现进度**：
- **系统管理模块**：87% (13/15 接口已实现) ⬆️
- **教务核心模块**：35% (14/40 接口已实现)
- **业务流程模块**：20% (5/25 接口已实现)
- **总体进度**：40% (32/80 接口已实现) ⬆️
...
**实现位置**: `UserManagementController.java:73`
...
**功能描述**: 添加新用户（管理员、教师、学生、家长）

**实现位置**: `UserManagementController.java:27`

**权限要求**: `@PreAuthorize("hasRole('super_admin')")`
...
| realName | String | 否 | 真实姓名，1-50个字符 |
...
**功能描述**: 删除用户（软删除，设置is_deleted=true）

**实现位置**: `UserManagementController.java:49`
...
**实现位置**: `UserManagementController.java:60`
...
**功能描述**: 获取所有角色列表（按排序和创建时间排序）

**实现位置**: `RoleController.java:77`
...
| roleCode | String | 是 | 角色编码，只能包含字母、数字和下划线 |
| roleDesc | String | 否 | 角色描述，最长200字符 |
| sortOrder | Integer | 否 | 显示排序，默认0 |
| status | Integer | 否 | 状态：0-禁用 1-启用，默认1 |
...
**功能描述**: 更新角色信息（不可修改roleCode）

**实现位置**: `RoleController.java:42`
...
**功能描述**: 删除角色（软删除，需检查用户关联）

**实现位置**: `RoleController.java:53`
...
- ✅ 软删除（设置is_deleted=true）
- ✅ 检查用户关联（有用户时不可删除）
- ✅ 超级管理员角色保护（不可删除super_admin）
- ✅ 超级管理员权限控制

**注意事项**:
- ✅ 不允许删除super_admin角色
- ✅ 如果角色下有用户，将提示用户数量并拒绝删除
...
**实现位置**: `UserManagementController.java:106`
...
**实现位置**: `UserManagementController.java:117`
...
**最后更新时间**: 2025-11-19
**版本**: v2.1
**更新内容**: 新增用户管理接口章节（1.2），包含用户列表查询、添加、修改、删除、详情查询5个接口
