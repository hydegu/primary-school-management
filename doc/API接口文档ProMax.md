...
- [二、教务管理](#二教务管理)
  ...
  - [6. 课程管理](#6-课程管理-菜单id-60)
    - [6.1 科目列表](#61-课程列表-菜单id-61)
    - [6.2 科目详情](#62-课程详情-菜单id-62)
...
**功能描述**: 查询学生列表，支持分页和多条件筛选

**请求参数**:
- `studentName` - 学生姓名（可选，模糊查询）
- `studentNo` - 学号（可选，模糊查询）
- `classId` - 班级ID（可选，精确匹配）
- `gradeId` - 年级ID（可选，精确匹配）
- `gender` - 性别（可选，精确匹配）
- `IdCard` - 身份证号（可选，精确匹配）
- `page` - 页码（可选，默认1）
- `size` - 每页条数（可选，默认10）
...
**功能描述**: 查询教师列表，支持模糊查询和条件筛选

**请求参数**:
- `teacherName` - 教师姓名（可选，模糊查询）
- `teacherNo` - 教师工号（可选，模糊查询）
- `title` - 职称（可选，模糊查询）
...
**功能描述**: 修改教师信息，支持更新基本信息、科目关联和班主任班级分配

**请求参数**:
```json
{
  "teacherNo": "T100",                           // 教师编号（可选）
  "teacherName": "李老师",                       // 教师姓名（可选）
  "gender": 1,                                   // 性别：1-男 2-女（可选）
  "birthDate": "1985-03-15",                     // 出生日期（可选）
  "idCard": "110101198503150011",                // 身份证号（可选）
  "phone": "13900139000",                        // 联系电话（可选）
  "email": "teacher100@school.com",              // 邮箱（可选）
  "title": "班主任",                             // 职称（可选，如：班主任/数学教师）
  "hireDate": "2024-09-01",                      // 入职日期（可选）
  "subjectIds": [1, 2],                          // 教授的科目ID列表（可选）
  "classId": 1                                   // 负责的班级ID（可选，仅班主任需要，一个班主任只能负责一个班级）
}
```

**业务规则**:
1. 所有字段均为可选，仅更新提供的字段
2. `teacherNo` 修改时会验证唯一性
3. `title` 设置为"班主任"时会自动更新用户角色为班主任角色
4. `subjectIds` 会先清除所有科目关联再重新添加
5. `classId` 会先清除该教师的所有班主任关联再分配新班级（一个班主任只能负责一个班级）：
   - 验证班级是否存在且未删除
   - 检查班级是否已有其他班主任
   - 如果分配班级但职称不是班主任会记录警告
...
**实现位置**: `TeacherController.java:63`
...
#### 🔘 6.1科目列表 (菜单ID: 65) ✅
> **说明**: 此模块管理学校的科目信息（语文、数学、英语等）及科目与教师的关联关系
...
**功能描述**: 查询所有科目，用于卡片展示（语文、数学、英语、体育、科学、道法、音乐、微机等）
  ...
  "dataset": [
    {
      "id": 1,                                     // 科目ID
      "subjectName": "语文",                       // 科目名称
      "subjectCode": "chinese",                    // 科目编码
      "sortOrder": 1,                              // 显示排序
      "status": 1,                                 // 状态：0-停用 1-启用
      "remark": "小学语文",                        // 备注
      "avatar": "https://example.com/chinese.png", // 封面图片URL
      "teacherIds": [1, 2, 3],                     // 授课教师ID列表
      "teacherCount": 3,                           // 授课教师数量
      "createdAt": "2025-11-15T12:24:40",         // 创建时间
      "updatedAt": "2025-11-15T12:24:40"          // 更新时间
    },
    {
      "id": 2,
      "subjectName": "数学",
      ...
      "status": 1,
      "remark": "小学数学",
      "avatar": "https://example.com/math.png",
      "teacherIds": [2, 4],
      "teacherCount": 2,
      "createdAt": "2025-11-15T12:24:40",
      "updatedAt": "2025-11-15T12:24:40"
    }
  ]
}
```
...
**接口地址**: `GET /api/subject/{id}/teachers`

**功能描述**: 查询指定科目的所有授课教师，用于在科目详情页展示
  ...
  "dataset": [
    {
      ...
      "gender": "女",                              // 性别
      "birthDate": "1985-05-15",                   // 出生日期
      "idCard": "110101198505150000",              // 身份证号
      "phone": "13800138001",                      // 手机号
      "email": "zhang@example.com",                // 邮箱
      "title": "班主任",                           // 职称
      "hireDate": "2010-09-01",                    // 入职日期
      "createdAt": "2025-11-20T10:00:00",         // 创建时间
      "updatedAt": "2025-11-20T10:00:00"          // 更新时间
    },
    {
...
**实现位置**: `SubjectController.java:80`
...
```json
{
  "subjectName": "思想品德",                    // 科目名称（必填）
  "subjectCode": "moral",                       // 科目编码（必填）
  "sortOrder": 11,                              // 显示排序（可选）
  "status": 1,                                  // 状态：0-停用 1-启用（可选，默认1）
  "remark": "小学思想品德",                     // 备注（可选）
  "avatar": "https://example.com/moral.png"     // 封面图片URL（可选）
}
```
...
```json
{
  ...
  "dataset": {
    ...
    "status": 1,
    "remark": "小学思想品德",
    "avatar": "https://example.com/moral.png",
    "teacherIds": [],
    "teacherCount": 0,
...
**实现位置**: `SubjectController.java:50`

---

#### 🔘 修改科目 ✅

**接口地址**: `PUT /api/subject/{id}`
...
```json
{
  "subjectName": "道德与法治",                  // 科目名称
  "subjectCode": "moral_law",                   // 科目编码
  "sortOrder": 12,                              // 显示排序
  "status": 1,                                  // 状态：0-停用 1-启用
  "remark": "道德与法治课程",                   // 备注
  "avatar": "https://example.com/moral_law.png" // 封面图片URL
}
```
...
```json
{
  ...
  "dataset": {
    "id": 9,
    "subjectName": "道德与法治",
    "subjectCode": "moral_law",
    "sortOrder": 12,
    "status": 1,
    "remark": "道德与法治课程",
    "avatar": "https://example.com/moral_law.png",
    "teacherIds": [1, 3],
    "teacherCount": 2,
    "createdAt": "2025-11-22T10:00:00",
    "updatedAt": "2025-11-22T10:15:00"
  }
}
```

**实现位置**: `SubjectController.java:61`

---

#### 🔘 删除科目 ✅

**接口地址**: `DELETE /api/subject/{id}`

**路由**: `/subject/delete`

**功能描述**: 删除科目（软删除），同时删除该科目与教师的关联关系
...
**实现位置**: `SubjectController.java:72`

---

#### 🔘 为科目添加授课老师 ✅

**接口地址**: `POST /api/subject/{id}/teachers`

**路由**: `/subject/addTeachers`
...
**实现位置**: `SubjectController.java:91`

---

#### 🔘 移除科目的授课老师 ✅

**接口地址**: `DELETE /api/subject/{id}/teachers`

**路由**: `/subject/removeTeachers`
...
**实现位置**: `SubjectController.java:107`

---

#### 🔘 上传科目封面 ✅

**接口地址**: `POST /api/subject/upload-cover`

**路由**: `/subject/uploadCover`

**功能描述**: 上传科目封面图片，返回图片访问URL

**请求参数**: `multipart/form-data`
- `file` - 图片文件（必填）
  - 支持格式：JPG、PNG、GIF
  - 最大大小：5MB
- `subjectId` - 科目ID（可选，用于文件命名）
...
```json
{
  ...
  "msg": "操作成功",
  "dataset": {
    "url": "/uploads/subject-covers/2025/11/22/1_abc123def456.jpg"
  }
}
```
...
**使用说明**:
1. 先调用此接口上传图片，获取返回的URL
2. 再调用添加/修改科目接口时，将URL作为avatar字段值传入

**实现位置**: `SubjectController.java:117`
